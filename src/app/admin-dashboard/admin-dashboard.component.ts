...
export class AdminDashboardComponent implements OnInit {
  ...
  getAllPuccaInputs() {
    this.http.get<any[]>(`https://176.186.145.154:3000/api/puccaInputs/all`)
      .subscribe({
        next: (res) => {          
  ...
  validateChanges(input: any) {
    ...
    this.http.put(`https://176.186.145.154:3000/api/puccaInputs/${input._id}`, cleanInput)
      .subscribe({
        next: (res) => {
  ...
  createPucca() {
    ...
    // }
    // });
    this.http.post(`https://176.186.145.154:3000/api/puccaInputs/new`, this.newPucca)
      .subscribe({
        next: (res) => {
  ...
  deleteInput(input: any) {
      ...
      this.http.delete(`https://176.186.145.154:3000/api/puccaInputs/${input._id}`)
        .subscribe({
          next: (res) => {
